...
# ---------------------------
# 2. Define Your Sample Video Files
# ---------------------------
# Provide the paths to the video files you wish to analyze.
sample_files = {
    "video": [
        "/Users/rofman/sdk/6d2buv.mp4"  # Replace with your actual video file path(s)
    ]
}

# Tip:
# For optimal performance, use high-quality uncompressed video files when possible.
...
def process_file(file_path, modality):
    ...
    which takes the file path and a list of modalities (in this case, just "video").
    It returns the job ID if the submission is successful.
    """
    try:
        response = deepid.process_file_async(
            file_path,
            run_description=False,  # Set to True if you want detailed analysis output
            modalities=[modality]   # Specify that we're processing a video file
        )
        return response['id']
...
def main():
    """
    Main function to process video files for deepfake detection.
    
    This function iterates through the sample_files dictionary,
    processing files under each modality (in this case, only "video").
    """

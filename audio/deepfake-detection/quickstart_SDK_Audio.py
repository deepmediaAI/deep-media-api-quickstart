...
# ---------------------------
# 2. Define Your Sample Audio Files
# ---------------------------
# Provide the paths to the audio files you wish to analyze.
sample_files = {
    "audio": [
        "/Users/rofman/sdk/presidential bedwars.wav"  # Replace with your actual audio file path(s)
    ]
}

# Tip:
# For optimal performance, use high-quality uncompressed audio files when possible.
...
def process_file(file_path, modality):
    ...
    which takes the file path and a list of modalities (in this case, just "audio").
    ...
    try:
        response = deepid.process_file_async(
            ...
            modalities=[modality]   # Specify that we're processing an audio file
        )
        return response['id']
...
def main():
    """
    Main function to process audio files for deepfake detection.
    
    This function iterates through the sample_files dictionary,
    processing files under the "audio" modality.
    """
